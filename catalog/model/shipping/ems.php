...
class ModelShippingEms extends Model
{
    ...
    function getQuote($address)
    {
        ...
        if ($this->config->get('ems_status')) {
            ...
            if ($emscity == "0") $city_from = $this->dest_check('FALSE', $query->row['name']);
                ...
                if ($checkemscity != "error") $city_from = $this->config->get('ems_city_from'); else $city_from = "error";
            }
            ...
            $city_to = $this->dest_check($address['zone_id'], $address['city']);
            ...
            if ($country_iso != 'RU') {
                $dops = '&type=att';
                $city_to = $this->country_check($country_iso);
            }
    ...
    function dest_check($str_reg, $str_city)
    {
        ...
        $RussiaList = $this->cache->get('zone.RussiaEms');
        // поиск по городу
        ...
        if (!$RussiaList) {
            ...
            if (empty($RussiaList['rsp']['err'] ))
                $this->cache->set('zone.RussiaEms', $RussiaList);
        }
        $reg = "";
        foreach ($RussiaList['rsp']['locations'] as  $val) {
            if ($val['name'] ==  mb_strtoupper($str_city, 'UTF-8')){
                $reg = $val['value'];
                break;
            }
...
//----------------
        ...
        if ($res != "error") {
            return mb_convert_case(mb_strtolower($res, 'utf-8'), MB_CASE_TITLE, 'utf-8');
        } else return $res;
